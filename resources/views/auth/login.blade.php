...
<head>
    ...
    <title>NEUROSPEND | Secure Access</title>
        ...
        <div class="max-w-xl w-full flex flex-col items-center md:items-start space-y-stack-md text-center md:text-left">
            ...
            <div class="space-y-stack-sm">
                ...
                <p class="font-body-md text-on-surface-variant max-w-md leading-relaxed text-[15px] md:text-body-md font-light">
                    A deterministic, local-first financial command center. Optimized cash runway computations, debt snowball scheduling, and zero-hallucination ledger intelligence.
                </p>
            </div>
            ...
            <div class="w-full max-w-sm hidden md:flex items-center space-x-stack-sm bg-surface-container-lowest/60 border border-outline-variant/10 px-4 py-3 rounded-none relative overflow-hidden">
                ...
                <div class="relative z-10 flex items-center space-x-3 w-full">
                    ...
                    <span id="log-stream" class="font-data-mono text-[11px] text-on-surface-variant truncate transition-opacity duration-300">Awaiting master key authorization...</span>
                </div>
            </div>
        </div>
    ...
    <section class="w-full md:w-2/5 lg:w-1/3 h-2/3 md:h-full bg-surface-dim relative z-10 flex flex-col justify-between p-container-padding border-t md:border-t-0 md:border-l border-outline-variant/20 overflow-y-auto">
        <div class="my-auto space-y-stack-md">
            ...
            <header class="space-y-stack-sm">
                <h2 class="font-headline-lg text-headline-lg text-primary tracking-tight">SECURE ACCESS</h2>
                <p class="font-body-md text-on-surface-variant text-[14px]">INPUT MASTER IDENTIFICATION CREDENTIALS</p>
            </header>
            ...
            @if($errors->any())
                <div class="bg-error-container/20 border border-error/30 text-error p-4 rounded-none space-y-1 animate-pulse">
                    <div class="flex items-center space-x-2">
                        ...
                        <span class="font-label-caps text-[11px] tracking-wide">AUTHENTICATION ERROR</span>
                    </div>
            ...
            @if(session('status'))
                <div class="bg-primary-fixed-dim/10 border border-primary-fixed-dim/30 text-primary-fixed-dim p-4 rounded-none space-y-1">
                    <div class="flex items-center space-x-2">
                        ...
                        <span class="font-label-caps text-[11px] tracking-wide">STATUS ACTIVE</span>
                    </div>
            ...
            @if(session('message'))
                <div class="bg-primary-fixed-dim/10 border border-primary-fixed-dim/30 text-primary-fixed-dim p-4 rounded-none space-y-1">
                    <div class="flex items-center space-x-2">
                        <span class="material-symbols-outlined text-[18px]">verified</span>
                        <span class="font-label-caps text-[11px] tracking-wide">SYSTEM TELEMETRY</span>
                    </div>
                    <p class="text-xs text-on-surface-variant">{{ session('message') }}</p>
                </div>
            @endif

            <form method="POST" action="/auth/login" class="space-y-stack-md">
                @csrf
                <!-- Email Field -->
                ...
                <div class="space-y-unit">
                    <div class="flex justify-between">
                        <label class="font-label-caps text-label-caps text-on-surface-variant ml-1">MASTER KEY</label>
                        <a class="font-label-caps text-[10px] text-primary-fixed-dim hover:underline uppercase" href="/auth/forgot-password">Revoke Access?</a>
                    </div>
                    <div class="relative">
                        <span class="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-on-surface-variant text-[18px]">fingerprint</span>
                        <input id="password" name="password" class="w-full h-14 bg-surface-container-lowest border border-outline-variant/30 px-12 font-data-mono text-data-mono text-primary rounded-none transition-all input-focus-ring" placeholder="••••••••••••" type="password" required/>
                        <button type="button" id="togglePasswordBtn" class="absolute right-4 top-1/2 -translate-y-1/2 text-on-surface-variant hover:text-primary-fixed transition-colors">
                            <span id="eyeIcon" class="material-symbols-outlined text-[18px]">visibility_off</span>
                        </button>
                    </div>
                </div>
                <!-- Remember Device -->
                <div class="flex items-center justify-between pt-unit">
                    <label class="flex items-center space-x-stack-sm cursor-pointer group">
                        <div class="relative w-10 h-5 bg-surface-container-high border border-outline-variant/30 rounded-full transition-colors group-hover:border-primary-fixed-dim">
                            <input class="sr-only peer" type="checkbox" name="remember" id="remember"/>
                            <div class="absolute left-1 top-1 w-2.5 h-2.5 bg-on-surface-variant rounded-full transition-all peer-checked:left-6 peer-checked:bg-primary-fixed-dim"></div>
                        </div>
                        <span class="font-label-caps text-[10px] text-on-surface-variant uppercase tracking-wider">Remember Hardware ID</span>
                    </label>
                </div>
                <!-- CTA -->
                <button type="submit" class="w-full h-16 bg-primary-fixed text-on-primary-fixed font-label-caps text-[13px] tracking-widest uppercase mt-gutter active:scale-[0.98] transition-all relative overflow-hidden group shadow-[0_0_30px_rgba(82,255,172,0.1)] hover:shadow-[0_0_40px_rgba(82,255,172,0.2)]">
                    <span class="relative z-10 flex items-center justify-center space-x-stack-sm">
                        <span>Decrypt &amp; Enter Command Center</span>
            ...
            <footer class="pt-section-gap flex flex-col items-center space-y-gutter">
                <p class="text-on-surface-variant font-label-caps text-[10px] opacity-70 uppercase tracking-widest">
                    Authorized Access Only. New Operator? <a href="/auth/register" class="text-primary-fixed-dim hover:underline font-bold">Deploy Node</a>
                </p>
...
<script>
    ...
    const logs = [
        ...
        "Encrypting terminal connection...",
        "Synching neural ledger...",
        "Bypassing generic protocols...",
        "Command Center Ready.",
        "Waiting for Master Key..."
    ];
    let logIndex = 0;
    ...
    if (togglePasswordBtn && passwordInput && eyeIcon) {
        ...
        togglePasswordBtn.addEventListener('click', () => {
            ...
            eyeIcon.textContent = isPasswordVisible ? 'visibility' : 'visibility_off';
        });
    }
</script>
</body>
